...
export const _metaConllLinesToJson = (metaConllLines: string[]): metaJson_T => {
  ...
  for (const metaCouple of metaConllLines) {
    const index = metaCouple.indexOf(' = ');
    if (index !== -1) {
      // if the line doesn't contains " = ", it is ignored (consitent with conllup-py)
      const metaKey = metaCouple.slice(0, index);
      const metaValue = metaCouple.slice(index + 3);
      const trimmedMetaKey = metaKey.slice(2); // remove "# " at the begining
